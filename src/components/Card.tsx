...
import { GripVertical, AlertCircle, Wand2, Edit3, Trash2 } from 'lucide-react';
...
export function Card({ tarjeta, allCards, isOverlay, isSubtask = false }: CardProps) {
  ...
  return (
    <>
      ...
      {subtasks.length > 0 && !isOverlay && !isDragging && (
        <div className="flex flex-col">
          {subtasks.map(sub => (
            <Card key={sub.id} tarjeta={sub} allCards={allCards} isSubtask={true} />
          ))}
        </div>
      )}
